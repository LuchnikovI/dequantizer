        ...
        while nodes_queue:
            ...
            if curr_label != dst:
                if curr_label not in visited_labels:
                    ...
                    for neighboring_label in filter(lambda x: (curr_label, x) != (src, dst), graph[curr_label]):
...
def shortest_cycles_distr(pairs: List[Tuple[int, int]]) -> List[float]:
    ...
    for (curr_label, neighboring_label) in pairs:
        if (curr_label, neighboring_label) not in visited_edges and (neighboring_label, curr_label) not in visited_edges:
            length = _shortest_cycle_length_trough_edge(graph, curr_label, neighboring_label)
            if len(cycle_lengths_stat) > length:
                cycle_lengths_stat[length] += 1. / edges_number
            else:
                curr_length = len(cycle_lengths_stat)
                cycle_lengths_stat.extend([0.] * (length - curr_length) + [1. / edges_number])
